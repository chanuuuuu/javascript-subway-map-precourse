...
#### 1. storage 접근 객체
  > localStorage를 하나의 DB로 생각하고 접근할 객체를 구현 
 
 </br>

#### 2. 역을 관리할 역 리스트 객체
  > 현재까지 유저가 저장한 역들을 저장할 객체
  1. 역 리스트 조회
...
#### 4. 노선 객체 내부에 저장될 구간 객체
> 유저가 저장한 노선에 역을 추가하여 구간을 생성하는 객체

1. 구간에 존재하는 모든 역 조회
  
2. 구간에 대한 순서 저장 
    - 링크드 리스트로 구현하여 중간에 삽입 가능하도록 구현

3. 구간 삭제
   - 구간 내에 2개의 역 존재시 불가 확인
...
구현의 절차를 서술하고 완료된 것은 줄을 그어서 표현하였다.
...
- 미션은 [https://github.com/woowacourse/javascript-subline-map-precours](https://github.com/woowacourse/javascript-subline-map-precourse) 저장소를 fork/clone해 시작한다.
- **기능을 구현하기 전에 javascript-subline-precourse/docs/README.md 파일에 구현할 기능 목록**을 정리해 추가한다.
